...
it('binds verification to Git state and carries a reviewed handoff through export', async () => {
  ...
  try {
    ...
    const report = await app.verify([{ command: 'test-tool', args: ['check'] }], { execute: async () => ({ code: 0, output: 'pass' }) });
...
it('records failed commands and audits historical privacy risks without exposing values', async () => {
  ...
  try {
    ...
    const report = await app.verify([{ command: 'missing-tool', args: [] }], { execute: async () => { throw new Error('not installed'); } });
    expect(report.passed).toBe(false);
    expect(report.results[0]?.exitCode).toBe(127);
    store.addRecord({ id: 'legacy-secret', sessionId: session.id, runId: null, kind: 'note', title: 'token=private-value', body: '', status: 'info', createdAt: '2026-01-01' });
    store.addRecord({ id: 'legacy-file', sessionId: session.id, runId: null, kind: 'file', title: '.env', body: '', status: 'info', createdAt: '2026-01-01' });
