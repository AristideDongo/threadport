...
describe('record deletion', () => {
  it('keeps unrelated events whose message matches the record title', () =>
    withStore((store, dir) => {
...
describe('store transactions', () => {
  it('lets nested writes join the outer transaction and roll back together', () =>
    withStore((store) => {
      store.createSession({
        id: 's1',
        title: 'Outer',
...
describe('redaction', () => {
  ...
  it('keeps the URL host when removing embedded credentials', () => {
    expect(redact('git clone https://alice:hunter2@github.com/org/repo')).toBe(
...
describe('partial Git state', () => {
  it('never produces a fingerprint and warns in the context', () =>
    withStore((store, dir) => {
      const git = {
        branch: 'main',
        head: 'abc',
        changedFiles: [],
        diff: '',
        warning: 'Too many changed or untracked files to read.',
      };
...
describe('CLI', () => {
  const run = (cwd: string, env: NodeJS.ProcessEnv, ...args: string[]) =>
    spawnSync(process.execPath, ['--import', loader, cli, ...args], {
      cwd,
      encoding: 'utf8',
      timeout: 20_000,
      env: { ...process.env, ...env },
  ...
  it('finds the project from a subdirectory, keeps data out of Git, and hides the SQLite warning', () => {
    ...
    try {
      ...
      expect(execFileSync('git', ['status', '--porcelain'], { cwd: dir, encoding: 'utf8' })).not.toContain(
        '.threadport',
      );
  ...
  it('refuses to run project agent manifests until the user trusts them', () => {
    ...
    try {
      ...
      writeFileSync(
        manifest,
        JSON.stringify({ version: 1, id: 'local', label: 'Local', command: 'git', args: ['status'], capabilities: [] }),
      );
