...
it('installs hooks without replacing existing settings and captures only allowed file metadata', () => {
  ...
  try {
    ...
    const config = JSON.parse(readFileSync(path, 'utf8')) as { permissions: { allow: string[] }; hooks: { PostToolUse: unknown[] } };
    ...
    handleLifecycleHook(app, dir, 'claude', { hook_event_name: 'PostToolUse', tool_name: 'Write', tool_input: { file_path: join(dir, '.env') }, tool_response: { content: 'password: private' } });
    handleLifecycleHook(app, dir, 'claude', { hook_event_name: 'PostToolUse', tool_name: 'Write', tool_input: { file_path: join(dir, 'src', 'main.ts') }, tool_response: { content: 'password: private' } });
