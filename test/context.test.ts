...
describe('context privacy', () => {
  it('excludes sensitive paths and their complete diff blocks', () => {
    ...
    const context = buildContext(
      store,
      session,
      { branch: 'main', head: 'abc', changedFiles: ['.env', 'src/a.ts'], diff },
      'deep',
      ['.env'],
    );
...
it('keeps some open tasks when their section exceeds the token budget', () => {
  const tasks: WorkRecord[] = Array.from({ length: 30 }, (_, index) => ({
    id: `t${index}`,
    sessionId: 's1',
    runId: null,
    kind: 'task',
...
it('warns when a stored summary was made against a different Git state', () => {
  const summary: WorkRecord = {
    id: 'sum',
    sessionId: 's1',
    runId: null,
    kind: 'summary',
    title: 'Summary',
    body: 'Done\nGit fingerprint: 0000000000000000',
    status: 'info',
    createdAt: '2026-01-01',
  };
  ...
  const pack = buildContextPack(
    summaryStore,
    session,
    { branch: 'main', head: 'abc', changedFiles: [], diff: '' },
    'standard',
    [],
  );
