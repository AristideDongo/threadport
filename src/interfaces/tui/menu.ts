  ...
  try {
    while (true) {
      ...
      if (choice === '1') for (const item of app.sessions()) console.log(`${item.id} ${item.status.padEnd(6)} ${item.title}`);
        ...
        if (!commands.length) console.log('No checks configured. Add verification.commands to .threadport/config.json.');
        else { const report = await app.verify(commands, new LocalCommandExecutor()); console.log(report.passed ? '✓ Verification passed.' : '✗ Verification failed or Git state changed.'); }
      }
