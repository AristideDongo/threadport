...
function routes(app: ThreadPort, cwd: string): Map<string, Handler> {
  const ok = { ok: true };
  const active = <T>(read: (id: string) => T[]): T[] => { const current = app.active(); return current ? read(current.id) : []; };
  const record = (kind: 'note' | 'task' | 'memory'): Handler => (body) => {
  ...
  return new Map<string, Handler>([
    ...
    ['POST /v1/decisions', (body) => { const input = recordInput.parse(body); app.decide(input.title, input.body ?? ''); return [201, ok]; }],
    ...
    ['POST /v1/sessions/rename', (body) => { const { id, title } = renameInput.parse(body); return [200, app.rename(id, title)]; }],
    ['POST /v1/sessions/delete', (body) => { app.deleteSession(idInput.parse(body).id); return [200, ok]; }],
    ['POST /v1/tasks/complete', (body) => [200, app.completeTask(idInput.parse(body).id)]],
    ['POST /v1/records/update', (body) => { const { id, title, body: details } = updateRecordInput.parse(body); return [200, app.updateRecord(id, title, details)]; }],
    ['POST /v1/records/delete', (body) => { app.deleteRecord(idInput.parse(body).id); return [200, ok]; }],
    ['POST /v1/summary', () => [201, app.summarize()]],
    ['POST /v1/handoff', (body) => [201, app.saveHandoff(handoffInput.parse(body).content)]],
    ['POST /v1/links', (body) => { const link = linkInput.parse(body); return [201, app.linkWork(link.kind, link.url)]; }],
    ...
    ['GET /v1/context', (_body, url) => [200, app.contextPack(assertMode(url.searchParams.get('mode') ?? readConfig(cwd).context.defaultMode), loadExcludes(cwd))]],
...
async function handle(request: IncomingMessage, response: ServerResponse, table: Map<string, Handler>): Promise<void> {
  try {
    ...
    if (!handler) { respond(response, 404, { error: 'Not found' }); return; }
    ...
    respond(response, error instanceof PayloadTooLarge ? 413 : 400, { error: error instanceof Error ? error.message : String(error) });
  }
}
