...
export function registerForkCommands(cli: Command): void {
  ...
  forkCommand
    ...
    .action(() => {
      const { store } = project();
      try {
        for (const item of new Forks(store, git, worktrees, cwd).list())
          console.log(`${item.id} ${item.agentId.padEnd(8)} ${item.branch} ${item.path}`);
      } finally {
        store.close();
      }
    });
  forkCommand
    .command('remove <id>')
    .description('Remove a clean worktree and keep its branch')
    .action((id: string) => {
      const { store } = project();
      try {
        new Forks(store, git, worktrees, cwd).remove(id);
        console.log(`✓ Worktree ${id} removed; branch kept.`);
      } finally {
        store.close();
      }
    });
  forkCommand
    .command('run <id>')
    .description('Launch the assigned agent in its worktree')
    .option('--structured', 'Capture provider JSON events')
    .action(async (id: string, options: { structured?: boolean }) => {
      const { store } = project();
      try {
        const fork = new Forks(store, git, worktrees, cwd).get(id);
        await launchIn(
          new ThreadPort(store, git, fork.path, loadExcludes(cwd)),
          fork.agentId,
  ...
  forkCommand
    .command('check <id> <executable> [args...]')
    .description('Run a command in a fork and record its result')
    .action(async (id: string, executable: string, args: string[]) => {
      const { store } = project();
      try {
        const fork = new Forks(store, git, worktrees, cwd).get(id);
        const code = await new ThreadPort(store, git, fork.path, loadExcludes(cwd)).check(
          executable,
          args,
  ...
  cli
    ...
    .description('Compare changes between two forks')
    .option('--diff', 'Show full diffs (size limited)')
    .action((first: string, second: string, options: { diff?: boolean }) => {
      const { store } = project();
      try {
