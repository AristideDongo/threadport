...
import { cwd, dbPath, findAgent, git, knownAgents, projectDir, requireActive, runner, withProjectAsync } from '../runtime.js';
...
export async function launchIn(app: ThreadPort, agentId: string, structured = false, providerSessionId: string | null = null): Promise<void> {
...
export function registerAgentCommands(cli: Command): void {
  cli.command('agents').description('List configured agents').action(() => {
    for (const agent of knownAgents()) console.log(`${runner.available(agent.command) ? '✓' : '·'} ${agent.id.padEnd(8)} ${agent.label} (${agent.command})${'trusted' in agent && !agent.trusted ? ' · untrusted' : ''}`);
  });
  ...
  plugin.command('list').description('List project agent adapters').action(() => {
    for (const item of loadAgentPlugins(projectDir)) console.log(`${item.id} ${item.command}${item.trusted ? '' : ' (untrusted)'}`);
  });
  plugin.command('trust <id>').description('Allow a reviewed project agent manifest to run').action((id: string) => {
