...
export function registerRecordCommands(cli: Command): void {
  cli
    ...
    .option('-r, --rationale <text>', 'Reason for the decision', '')
    .action((title: string, options: { rationale: string }) =>
      withProject((app) => {
  ...
  cli
    ...
    .action(() =>
      withProject((app) => {
        for (const item of app.decisions(requireActive(app).id))
          console.log(`• ${item.title}${item.rationale ? ` — ${item.rationale}` : ''}`);
      }),
    );
  cli
    .command('note <text>')
    .description('Record a work note')
    .action((value: string) =>
      withProject((app) => {
  ...
  recordCommand
    ...
    .description('Edit a record title and optional details')
    .option('-d, --details <text>', 'Replace record details')
    .action((id: string, title: string, options: { details?: string }) =>
      withProject((app) => {
  ...
  task
    ...
    .action(() =>
      withProject((app) => {
        for (const item of app.records(requireActive(app).id).filter((entry) => entry.kind === 'task'))
          console.log(`${item.status === 'done' ? '✓' : '○'} ${item.id} ${item.title}`);
      }),
    );
  cli
    .command('error <title>')
    .description('Record an error')
    .option('-d, --details <text>', 'Details', '')
    .action((title: string, options: { details: string }) =>
      withProject((app) => {
  ...
  cli
    .command('test-result <title>')
    .description('Record a test result')
    .requiredOption('-s, --status <status>', 'passed or failed')
    .option('-d, --details <text>', 'Details', '')
    .action((title: string, options: { status: string; details: string }) =>
      withProject((app) => {
  ...
  cli
    .command('check <executable> [args...]')
    .description('Run a command and record its result')
    .action(async (executable: string, args: string[]) =>
  ...
  cli
    ...
    .description('Record a command run elsewhere')
    .option('-d, --details <text>', 'Result', '')
    .action((value: string, options: { details: string }) =>
      withProject((app) => {
  ...
  cli
    .command('memory <text>')
    .description('Add durable project knowledge')
    .action((value: string) =>
      withProject((app) => {
  ...
  cli
    ...
    .description('Record a session constraint')
    .action((value: string) =>
      withProject((app) => {
  ...
  cli
    .command('file <path>')
    .description('Mark a relevant file')
    .option('-d, --details <text>', 'Reason', '')
    .action((path: string, options: { details: string }) =>
      withProject((app) => {
  ...
  cli
    .command('files')
    .description('List relevant and changed files')
    .action(() =>
      withProject((app) => {
  ...
  cli
    ...
    .description('Record a work artifact')
    .option('-d, --details <text>', 'Description', '')
    .action((path: string, options: { details: string }) =>
      withProject((app) => {
