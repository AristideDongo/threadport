...
import { contextMode, cwd, git, invocationDir, knownAgents, project, requireActive, runner, withProject } from '../runtime.js';
...
export function registerSessionCommands(cli: Command): void {
  cli.command('init').description('Initialize ThreadPort in the current directory').action(() => {
    if (existsSync(join(invocationDir, projectDirName, databaseFileName))) { console.log('✓ ThreadPort is already initialized here.'); return; }
