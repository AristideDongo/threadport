...
export function registerSystemCommands(cli: Command): void {
  ...
  config
    ...
    .action(() => console.log(JSON.stringify(readConfig(cwd), null, 2)));
  config
    .command('set-default-mode <mode>')
    .description('Set the default context mode')
    .action((value: string) =>
      withProject(() => {
  ...
  privacy
    .command('audit')
    .description('Preview redaction and excluded references before sharing')
    .option('--show-context', 'Print the redacted context pack')
    .action((options: { showContext?: boolean }) =>
      withProject((app) => {
        const mode = readConfig(cwd).context.defaultMode;
        const report = app.privacyAudit(mode);
        console.log(
          `Redaction candidates: ${report.redactedFields}\nExcluded references: ${report.excludedReferences}\nContext tokens: ${report.contextTokens}`,
        );
  ...
  privacy
    .command('scrub')
    .description('Redact existing records and remove excluded file references')
    .action(() =>
      withProject((app) => {
  ...
  cli
    ...
    .action(() => {
      console.log(
        `Node     ${process.version}${Number(process.versions.node.split('.')[0]) >= 24 ? ' ✓' : ' (Node 24 required)'}`,
      );
      ...
      for (const agent of knownAgents()) {
        ...
        if ('trusted' in agent && !agent.trusted) {
          console.log(`${agent.label.padEnd(11)} untrusted · threadport plugin trust ${agent.id}`);
          continue;
        }
        console.log(
          `${agent.label.padEnd(11)} ${runner.available(agent.command) ? `${agentVersion(agent.command) ?? 'available'} ✓` : `missing${guide}`}`,
        );
      }
      console.log(`Checks   ${verificationCommands(cwd).length} configured or detected`);
      console.log(
        `Hooks    Claude ${existsSync(join(cwd, '.claude', 'settings.local.json')) ? 'configured' : 'optional'} · Codex ${existsSync(join(cwd, '.codex', 'hooks.json')) ? 'configured' : 'optional'}`,
      );
  ...
  cli
    ...
    .action(async () => withProjectAsync((app) => openMenu(app, cwd)));
  cli
    .command('serve')
    .description('Start a local API with a temporary token')
    .option('-p, --port <port>', 'TCP port', '0')
    .action(async (options: { port: string }) =>
      withProjectAsync((app) => {
  ...
  mcp
    .command('setup')
    .description('Show commands to connect Claude Code or Codex to this project')
    .action(() => {
