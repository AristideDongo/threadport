...
export function registerContinuityCommands(cli: Command): void {
  ...
  handoff
    .command('draft')
    .description('Show a sourced handoff draft')
    .option('-o, --out <file>', 'Write a new Markdown file for review')
    .action((options: { out?: string }) =>
      withProject((app) => {
        const draft = app.handoffDraft();
        if (options.out) {
          writeFileSync(options.out, `${draft}\n`, { mode: 0o600, flag: 'wx' });
          console.log(`✓ Handoff draft: ${resolve(options.out)}`);
        } else console.log(draft);
      }),
    );
  handoff
    .command('save <file>')
    .description('Save a Markdown handoff from a file')
    .action((file: string) =>
      withProject((app) => {
  ...
  handoff
    .command('show')
    .description('Show the latest saved handoff')
    .action(() =>
      withProject((app) => {
  ...
  link
    ...
    .action((url: string) => withProject((app) => console.log(`✓ Linked ${app.linkWork('issue', url).title}`)));
  link
    .command('pr <url>')
    .description('Link a GitHub pull request')
    .action((url: string) => withProject((app) => console.log(`✓ Linked ${app.linkWork('pr', url).title}`)));
  cli
    .command('verify [executable] [args...]')
    .description('Run configured checks, or one supplied command, and bind results to Git state')
    .action(async (executable: string | undefined, args: string[], _options: unknown) =>
      withProjectAsync(async (app) => {
        const commands = executable ? [{ command: executable, args }] : verificationCommands(cwd);
        if (!commands.length)
          throw new Error(
            'No verification commands. Add verification.commands to .threadport/config.json or pass an executable.',
          );
        const report = await app.verify(commands, new LocalCommandExecutor());
        for (const result of report.results)
          console.log(
            `${result.exitCode === 0 ? '✓' : '✗'} ${[result.command, ...result.args].join(' ')} (exit ${result.exitCode}, ${result.durationMs} ms)`,
          );
        console.log(
          report.passed
            ? '✓ Verification passed on the current Git state.'
            : report.unchanged
              ? '✗ Verification failed.'
              : '⚠ Verification cannot be tied to the current Git state.',
        );
  ...
  cli
    ...
    .description('Search sessions, decisions, and records')
    .action((query: string) =>
      withProject((app) => {
  ...
  cli
    .command('export <id>')
    .description('Export a session as portable JSON')
    .requiredOption('-o, --out <file>', 'Destination file')
  ...
  cli
    .command('import <file>')
    .description('Import a JSON session archive')
    .action((file: string) => {
