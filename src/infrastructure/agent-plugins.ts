...
export interface PluginAgent extends AgentAdapter {
...
function manifest(value: unknown): AgentManifest {
  ...
  if (
    ...
    throw new Error('Invalid agent manifest: version, id, label, command, args, and capabilities are required.');
...
function trustPath(): string {
  return join(process.env.THREADPORT_HOME ?? join(homedir(), '.threadport'), 'trusted-agents.json');
}
  ...
  return readdirSync(directory)
    ...
    .map((name) => {
      ...
      return {
        ...
        manifestPath,
        trusted: trusted[manifestPath] === digest(content),
        args: (contextFile: string) => value.args.map((arg) => arg.replaceAll('{contextFile}', contextFile)),
      };
    });
}
...
export function assertTrusted(agent: AgentAdapter): void {
  if ('trusted' in agent && agent.trusted === false) {
    ...
    throw new Error(
      `Agent ${plugin.id} runs "${plugin.command}" from ${plugin.manifestPath}, which you have not trusted. Review the manifest, then run "threadport plugin trust ${plugin.id}".`,
    );
  }
}
