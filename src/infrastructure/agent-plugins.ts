...
function manifest(value: unknown): AgentManifest {
  if (typeof value !== 'object' || value === null) throw new Error('Manifest agent invalide.');
  const item = value as Record<string, unknown>;
  if (item.version !== 1 || typeof item.id !== 'string' || !/^[a-z][a-z0-9_-]{1,30}$/.test(item.id) || typeof item.label !== 'string' || typeof item.command !== 'string' || !item.command || !Array.isArray(item.args) || !item.args.every((arg) => typeof arg === 'string') || !Array.isArray(item.capabilities) || !item.capabilities.every((capability) => typeof capability === 'string')) throw new Error('Manifest agent invalide : version, id, label, command, args et capabilities sont requis.');
