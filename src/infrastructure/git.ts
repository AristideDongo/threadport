...
export class GitCliReader implements GitReader {
  read(cwd: string): GitState | null {
    try {
      ...
      let entries: string[];
      try {
        entries = git(cwd, ['status', '--porcelain=v1', '-z', '-uall'], statusLimit).split('\0').filter(Boolean);
      } catch (error: unknown) {
        ...
        return {
          ...
          changedFiles: [],
          diff: '',
          warning: 'Too many changed or untracked files to read. Add large generated folders to .gitignore.',
        };
      }
      ...
      try {
        ...
        return {
          ...
          warning: 'The Git diff is too large or unavailable; verification cannot be tied to it.',
        };
      }
