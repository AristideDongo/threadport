...
export function installLifecycleHooks(root: string, provider: Provider): string {
  ...
  for (const event of ['SessionStart', 'PostToolUse', 'SessionEnd']) {
    ...
    if (!present)
      list.push({
        ...(event === 'PostToolUse'
          ? { matcher: '^(Write|Edit|MultiEdit|ApplyPatch|apply_patch|functions\\.apply_patch)$' }
          : {}),
...
export function handleLifecycleHook(app: ThreadPort, root: string, provider: Provider, input: unknown): string | null {
  ...
  if (name === 'PostToolUse') {
    ...
    if (!['Write', 'Edit', 'MultiEdit', 'ApplyPatch', 'apply_patch', 'functions.apply_patch'].includes(tool))
      return null;
    const inputObject = object(event.tool_input);
    const rawPath = string(inputObject?.file_path) || string(inputObject?.path);
    if (!rawPath) {
      app.addRecord('note', `${provider} ${tool} completed`);
      return null;
    }
    ...
    if (
      ...
      path === '..' ||
      path.startsWith(`..${sep}`) ||
      excluded(path.replaceAll('\\', '/'), loadExcludes(root))
    )
      return null;
