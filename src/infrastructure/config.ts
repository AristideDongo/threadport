...
export interface ProjectConfig {
  context: { defaultMode: ContextMode };
  privacy: { exclude: string[] };
...
export function readConfig(cwd: string): ProjectConfig {
  ...
  if (!existsSync(path))
    return { context: { ...defaults.context }, privacy: { exclude: [] }, verification: { commands: [] } };
  const raw: unknown = JSON.parse(readFileSync(path, 'utf8'));
  if (typeof raw !== 'object' || raw === null) throw new Error('Invalid ThreadPort configuration.');
  const value = raw as Record<string, unknown>;
  const context =
    typeof value.context === 'object' && value.context !== null ? (value.context as Record<string, unknown>) : {};
  const privacy =
    typeof value.privacy === 'object' && value.privacy !== null ? (value.privacy as Record<string, unknown>) : {};
  const verification =
  ...
  if (!Array.isArray(exclude) || !exclude.every((item) => typeof item === 'string'))
    throw new Error('privacy.exclude must be a list of paths.');
  const commands = verification.commands ?? [];
  if (
    !Array.isArray(commands) ||
    !commands.every((item: unknown) => {
  ...
  return {
    context: { defaultMode: mode },
    privacy: { exclude },
    verification: { commands: commands as VerificationCommand[] },
  };
}
...
export function verificationCommands(cwd: string): VerificationCommand[] {
  ...
  return ['check', 'test', 'build']
    ...
    .map((name) => ({ command: 'npm', args: ['run', name] }));
}
