...
export function readConfig(cwd: string): ProjectConfig {
  ...
  if (typeof raw !== 'object' || raw === null) throw new Error('Configuration ThreadPort invalide.');
  ...
  if (!Array.isArray(exclude) || !exclude.every((item) => typeof item === 'string')) throw new Error('privacy.exclude doit être une liste de chemins.');
