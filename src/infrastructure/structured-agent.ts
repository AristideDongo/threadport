...
export function interpretAgentEvents(agentId: string, value: unknown): CapturedAgentEvent[] {
  ...
  if (agentId === 'codex') {
    ...
    if (item.type === 'file_change') return [{ kind: 'message', title: 'Codex file changes', body: JSON.stringify(item.changes ?? []).slice(0, 20000) }];
  }
  if (agentId === 'claude') {
    ...
    if (type === 'assistant') {
      ...
      return content.flatMap((block: unknown): CapturedAgentEvent[] => {
        ...
        if (item?.type === 'text' && string(item.text)) return [{ kind: 'message', title: 'Claude output', body: string(item.text).slice(0, 20000) }];
        if (item?.type === 'tool_use' && item.name === 'Bash') return [{ kind: 'command', title: string(object(item.input)?.command).slice(0, 200), body: '' }];
        return [];
      });
    }
    if (type === 'result') {
      const result: CapturedAgentEvent[] = [{ kind: event.is_error === true ? 'error' : 'message', title: 'Claude result', body: string(event.result).slice(0, 20000) }];
      if (object(event.usage)) result.push({ kind: 'usage', title: 'Claude usage', body: JSON.stringify(event.usage).slice(0, 2000) });
      return result;
    }
  }
  return [];
  ...
  run(command: string, args: string[], cwd: string): Promise<number> {
    return new Promise((resolve, reject) => {
      ...
      child.once('close', (code) => {
        if (failed) return;
        try { consume(pending); resolve(code ?? 1); } catch (error: unknown) { reject(error); }
      });
    });
  }
}
