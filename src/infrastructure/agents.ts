...
const prompt = (file: string) =>
  `Read the ThreadPort handoff at ${file} and continue the task from the current project state.`;
export const adapters: readonly AgentAdapter[] = [
  {
    id: 'claude',
    label: 'Claude Code',
    command: 'claude',
    capabilities: ['filesystem_access', 'git_access', 'terminal_access'],
    args: (file) => [prompt(file)],
    structuredArgs: (file) => ['-p', '--output-format', 'stream-json', '--verbose', prompt(file)],
    resumeArgs: (id, file) => ['--resume', id, prompt(file)],
  },
  {
    id: 'codex',
    label: 'Codex CLI',
    ...
    capabilities: ['filesystem_access', 'git_access', 'terminal_access'],
    args: (file) => [prompt(file)],
    structuredArgs: (file) => ['exec', '--json', prompt(file)],
    resumeArgs: (id, file) => ['resume', '--include-non-interactive', id, prompt(file)],
  },
];
...
export function adapterById(id: string, extra: readonly AgentAdapter[] = []): AgentAdapter {
  ...
  if (!adapter)
    throw new Error(
      `Unknown agent: ${id}. Available agents: ${[...adapters, ...extra].map((item) => item.id).join(', ')}.`,
    );
  return adapter;
}
