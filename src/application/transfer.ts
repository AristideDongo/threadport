...
const recordSchema = z.object({
  ...
  title: z.string(),
  body: z.string(),
  status: z.enum(['open', 'done', 'failed', 'info']),
  createdAt: z.string(),
});
...
const archiveV1Schema = z.object({
  ...
  runs: z.array(legacyRunSchema).max(100_000),
  events: z.array(eventSchema).max(100_000),
  decisions: z.array(decisionSchema).max(100_000),
  records: z.array(legacyRecordSchema).max(100_000),
  snapshots: z.array(snapshotSchema).max(100_000),
});
const archiveV2Schema = archiveV1Schema.extend({
  formatVersion: z.literal(2),
  runs: z.array(runSchema).max(100_000),
...
export class SessionTransfer {
  ...
  export(id: string): SessionArchive {
    ...
    return {
      ...
      runs: this.store
        .listRuns(id)
        .filter((run) => !run.forkId)
        .map(({ ownerPid: _ownerPid, forkId: _forkId, ...run }) => run),
      events: this.store
        .listEvents(id)
        .filter((event) => !excluded(event.message, this.patterns))
        .map((event) => ({ ...event, message: redact(event.message) })),
      decisions: this.store
        .listDecisions(id)
        .map((decision) => ({ ...decision, title: redact(decision.title), rationale: redact(decision.rationale) })),
      records: this.store
        .listRecords(id)
        .filter(
          (record) =>
            !record.forkId &&
            !((record.kind === 'file' || record.kind === 'artifact') && excluded(record.title, this.patterns)),
        )
        .map(({ forkId: _forkId, ...record }) => ({
          ...record,
          title: redact(record.title),
          body: redact(record.body),
        })),
      snapshots: this.store.listSnapshots(id).map((snapshot) => ({
        ...snapshot,
        git: {
          ...snapshot.git,
          changedFiles: snapshot.git.changedFiles.filter((path) => !excluded(path, this.patterns)).map(redact),
          diff: '',
        },
      })),
    };
  ...
  import(input: unknown): Session {
    ...
    const session: Session = {
      ...archive.session,
      id,
      title: redact(archive.session.title),
      status: 'active',
      updatedAt: now,
    };
    this.store.transaction(() => {
      ...
      for (const event of archive.events)
        if (!excluded(event.message, this.patterns))
          this.store.addEvent({ ...event, id: randomUUID(), sessionId: id, message: redact(event.message) });
      ...
      this.store.addEvent({
        ...
        message: `Imported from ${archive.session.id}`,
        createdAt: now,
      });
    });
    return session;
