...
export function parseVerification(body: string): VerificationReport | null {
  try {
    ...
    if (
      ...
      return null;
    if (
      !value.results.every((item: unknown) => {
        const result = object(item);
        return (
          result &&
          typeof result.command === 'string' &&
          Array.isArray(result.args) &&
          result.args.every((arg: unknown) => typeof arg === 'string') &&
...
export function githubWorkLink(kind: 'issue' | 'pr', raw: string): { title: string; url: string } {
  ...
  try {
    ...
    throw new Error('Provide a GitHub issue or pull request URL.');
  }
...
export function renderHandoff(input: {
  ...
  const lines = [
    ...
    ...(tasks.filter((item) => item.status === 'done').length
      ? tasks.filter((item) => item.status === 'done').map((item) => `- ${item.title} (record:${item.id})`)
      : ['- None recorded.']),
    '',
    '## Next steps',
    ...(tasks.filter((item) => item.status === 'open').length
      ? tasks.filter((item) => item.status === 'open').map((item) => `- ${item.title} (record:${item.id})`)
      : ['- None recorded.']),
    '',
    '## Changes and decisions',
    ...(git?.changedFiles.length
      ? git.changedFiles.map((path) => `- Changed: ${path}`)
      : ['- No uncommitted file changes.']),
    ...decisions
      .slice(-5)
      .map((item) => `- Decision: ${item.title}${item.rationale ? ` — ${item.rationale}` : ''} (decision:${item.id})`),
    ...
    ...(verification?.results.map(
      (result) =>
        `- ${[result.command, ...result.args].join(' ')}: exit ${result.exitCode} (record:${result.recordId})`,
    ) ?? []),
    '',
